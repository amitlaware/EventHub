...
void leftClipper()
{
    ...
    for (i = 0; i < verticesCount; i++)
    {
        ...
        if (points[i].x < clipWindowMin.x && points[i + 1].x >= clipWindowMin.x)
        {
            ...
            {
                clipPoints[clipVerticesCount].y = (points[i+1].y-points[i].y)/(points[i+1].x-
                points[i].x)*(clipWindowMin.x-points[i].x)+points[i].y;
            }
            else
            {
...
void rightClipper()
{
    ...
    for (i = 0; i < verticesCount; i++)
    {
        ...
        if(points[i].x > clipWindowMax.x && points[i+1].x <= clipWindowMax.x)
        {
            if(points[i+1].x - points[i].x != 0)
            {
                clipPoints[clipVerticesCount].y=(points[i+1].y-points[i].y)/(points[i+1].x-points[i].x) *
                (clipWindowMax.x-points[i].x) + points[i].y;
            }
            else
            {
                clipPoints[clipVerticesCount].y=points[i].y;
        ...
        if(points[i].x<=clipWindowMax.x && points[i+1].x>clipWindowMax.x)
        {
            ...
            {
                clipPoints[clipVerticesCount].y=(points[i+1].y-points[i].y)/(points[i+1].x-
                points[i].x)*(clipWindowMax.x-points[i].x)+points[i].y;
            }
            else
            {
                clipPoints[clipVerticesCount].y=points[i].y;
...
void topClipper()
{
    ...
    for(i=0;i<verticesCount;i++)
    {
        ...
        if(points[i].y>clipWindowMax.y && points[i+1].y<=clipWindowMax.y)
        {
            ...
            {
                clipPoints[clipVerticesCount].x=(points[i+1].x-points[i].x)/(points[i+1].y-
                points[i].y)*(clipWindowMax.y-points[i].y)+points[i].x;
            }
            else {
        ...
        if(points[i].y<=clipWindowMax.y && points[i+1].y>clipWindowMax.y)
        {
            ...
            {
                lipPoints[clipVerticesCount].x=points[i].x;
            }
            clipPoints[clipVerticesCount].y=clipWindowMax.y;
            clipVerticesCount++;
        }
...
void bottomClipper()
{
    ...
    for (i=0;i<verticesCount;i++)
    {
        ...
        if (points[i].y<clipWindowMin.y && points[i+1].y>=clipWindowMin.y)
        {
            ...
            {
                clipPoints[clipVerticesCount].x=(points[i+1].x-points[i].x)/(points[i+1].y-
                points[i].y)*(clipWindowMin.y-points[i].y)+points[i].x;
            }
            else
            {
                clipPoints[clipVerticesCount].x=points[i].x;
